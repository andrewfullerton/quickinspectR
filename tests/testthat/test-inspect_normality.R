...
test_that("advanced usage of inspect_normality returns a ggplot object with correct layers and facets", {
  # Generate plot and objects for use in testing
  test_plot <- inspect_normality(iris, vars = c("Sepal.Length", "Sepal.Width"), bins = 10)
  num_facets<- test_plot$data |> distinct(variable) |> nrow()
  histogram_layer <- test_plot$layers[[1]]
