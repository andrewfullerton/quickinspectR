...
Even though all `inspect` functions will run with `data` as their sole argument, there may be times when you want to stylize your plots a bit more. To make this more accessible, each `inspect` functions contain several additional (but completely optional) arguments to enable basic plot customization. Here's an example using `inspect_missing`:
...
And while advanced plot customization is *not* the goal of **quickinspectR**, all `quickinspectR` functions are built on top of ggplot2 and support more advanced customization via additional ggplot2-supported arguments. To learn more about this, you can read our vignette. 
